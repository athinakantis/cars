...
newListingForm.addEventListener('submit', (e) => {
    ...
    try {
        ...
        if (cars.find((car) => car.licensePlate === licenseInput.value.toUpperCase())) {
        ...
        if (makerInput.value === 'Default') {
            throw new Error(`Missing maker for new listing`)
        }
...
function updateListings(car, highlight = false) {
    ...
    deleteBtn.forEach((button, index) => button.addEventListener('click', () => deleteListing(index)))
}
...
function search() {
    const foundCar = cars.find((car) => car.licensePlate === searchBar.value.toUpperCase().trim());
    ...
    if (foundCar) {
        ...
        showMessage(`No car with license plate ${searchBar.value.toUpperCase()} found. Check spelling and try again`, 'error')
    }
}
...
//
document.querySelector('#redirectSearch').addEventListener('click', toggleSearch);
